...
class LyricHelper {
  ...
  /// 获取行间距
  static double getLineSpaceHeight(LyricsLineModel element, LyricUI ui, {bool excludeInline = false}) {
  ...
  static double _realCenterOffset(
    ...
    switch (baseLine) {
      case LyricBaseLine.MAIN_CENTER:
        return ((isCurr ? lyric.drawInfo?.playingMainTextHeight : lyric.drawInfo?.otherMainTextHeight) ?? 0) / 2;
      case LyricBaseLine.EXT_CENTER:
        if (!lyric.hasExt) {
          return _realCenterOffset(lyric, isCurr, LyricBaseLine.MAIN_CENTER, lyricUI, playIndex);
        }
        return ((isCurr ? lyric.drawInfo?.playingMainTextHeight : lyric.drawInfo?.otherMainTextHeight) ?? 0) +
            lyricUI.getInlineSpace() +
            ((isCurr ? lyric.drawInfo?.playingExtTextHeight : lyric.drawInfo?.otherExtTextHeight) ?? 0) / 2;

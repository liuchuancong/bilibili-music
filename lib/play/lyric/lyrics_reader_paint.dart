...
import 'package:bilibilimusic/play/lyric/lyric_helper.dart';
...
class LyricsReaderPaint extends ChangeNotifier implements CustomPainter {
  ...
  double get baseOffset => lyricUI.halfSizeLimit() ? mSize.height * (0.5 - lyricUI.getPlayingLineBias()) : 0;
  ...
  ///绘制其他歌词行
  ///返回造成的偏移量值
  double _drawOtherLyricLine(Canvas canvas, double drawOffsetY, LyricsLineModel element, int lineIndex) {
    var isPlay = lineIndex == playingIndex;
    var mainTextPainter = (isPlay ? element.drawInfo?.playingMainTextPainter : element.drawInfo?.otherMainTextPainter);
    var extTextPainter = (isPlay ? element.drawInfo?.playingExtTextPainter : element.drawInfo?.otherExtTextPainter);
    //该行行高
  ...
  void drawHighlight(LyricsLineModel model, Canvas canvas, TextPainter? painter, Offset offset) {
    ...
    model.drawInfo?.inlineDrawList.forEach((element) {
      ...
      canvas.drawRect(Rect.fromLTWH(dx, offset.dy + element.offset.dy - 2, currentWidth, element.height + 2),

...
import 'package:bilibilimusic/play/lyric/lyrics_log.dart';
import 'package:bilibilimusic/play/lyric/lyric_ui/lyric_ui.dart';
...
import 'package:bilibilimusic/play/lyric/lyrics_reader_paint.dart';
...
class LyricReaderState extends State<LyricsReader> with TickerProviderStateMixin {
  ...
  void scrollToPlayLine([bool animation = true]) {
    safeLyricOffset(
        widget.model?.computeScroll(lyricPaint.playingIndex, lyricPaint.playingIndex, widget.ui) ?? 0, animation);
  }
  ...
  void refreshLyricHeight(Size size) {
    ...
    widget.model?.lyrics.forEach((element) {
      var drawInfo = LyricDrawInfo()
        ..playingExtTextPainter = getTextPaint(element.extText, widget.ui.getPlayingExtTextStyle(), size: size)
        ..otherExtTextPainter = getTextPaint(element.extText, widget.ui.getOtherExtTextStyle(), size: size)
        ..playingMainTextPainter = getTextPaint(element.mainText, widget.ui.getPlayingMainTextStyle(), size: size)
        ..otherMainTextPainter = getTextPaint(element.mainText, widget.ui.getOtherMainTextStyle(), size: size);
        ...
        child: Center(
          child: StreamBuilder<int>(
              ...
              builder: (context, snapshot) {
                ...
                return widget.selectLineBuilder!.call(lyricPaint.model?.lyrics[centerIndex].startTime ?? 0, () {
                  setSelectLine(false);
                  disposeFiling();
  ...
  void resumeSelectLineOffset() {
    ...
    waitTimer = Timer.periodic(const Duration(milliseconds: 100), (timer) {
      ...
      if (waitSecond == 400) {
        realUpdateOffset(
            widget.model?.computeScroll(lyricPaint.centerLyricIndex, lyricPaint.playingIndex, widget.ui) ?? 0);
        return;
      }
      if (waitSecond == 3000) {
        disposeSelectLineDelay();
  ...
  void handleHighlight() {
    ...
    if (blankTime > 0) {
      items.add(TweenSequenceItem(tween: Tween(begin: 0.0, end: 0.0), weight: blankTime.toDouble()));
    }
    for (LyricSpanInfo element in spans) {
      ...
      items.add(TweenSequenceItem(
          tween: Tween(begin: begin, end: width += element.drawWidth), weight: element.duration.toDouble()));
    }
    disposeHighlight();
    if (items.isEmpty) {
    ...
    var animate = TweenSequence(items).chain(CurveTween(curve: Curves.easeInOut)).animate(_highlightController!)
      ..addStatusListener((status) {
        if (status == AnimationStatus.completed) {
          disposeHighlight();
        }
